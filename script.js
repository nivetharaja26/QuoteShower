...
function printQuote()
{
    ...
    var QuoteStr = `<p class="quote">"${quotes.quote}"</p><p class="source">-${quotes.source}`;

    if(quotes.citation)
    {
        QuoteStr += `<span class="citation">,${quotes.citation}</span>`
    }
    if(quotes.year)
    {
        QuoteStr += `<span class="year">,${quotes.year}.</span></p>`
    }
    else{
    ...
    QuoteContainer.innerHTML = QuoteStr;
...
document.getElementById("nextQuote").addEventListener("click",printQuote,false);